...
if($_POST){
	$save_dir=get_cfg('ly200.up_file_base_dir').'info2/'.date('y_m_d/', $service_time);
	$InfoId=(int)$_POST['InfoId'];
	$query_string=$_POST['query_string'];
	
	$Title=$_POST['Title'];
	$CateId=$db->get_row_count('info2_category')>1?(int)$_POST['CateId']:$db->get_value('info2_category', 1, 'CateId');
	...
	if(get_cfg('info2.upload_pic')){
		$S_PicPath=$_POST['S_PicPath'];
		if($BigPicPath=up_file($_FILES['PicPath'], $save_dir)){
			include('../../inc/fun/img_resize.php');
			$SmallPicPath=img_resize($BigPicPath, '', get_cfg('info2.pic_width'), get_cfg('info2.pic_height'));
	...
	set_page_url('info2', "InfoId='$InfoId'", get_cfg('info2.page_url'), 1);
...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="index.php"><?=get_lang('info2.info2_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php if($db->get_row_count('info2_category')>1){?>
		<tr>
			<td nowrap><?=get_lang('ly200.category');?>:</td>
			<td><?=ouput_Category_to_Select('CateId', $info_row['CateId'], 'info2_category', 'UId="0,"', 1, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	...
	<?php }?>
	<?php if(get_cfg('info2.upload_pic')){?>
		<tr>
	...
	<?php }?>
	<?php if(get_cfg('info2.is_in_index') || get_cfg('info2.is_hot')){?>
		<tr>
			<td nowrap><?=get_lang('ly200.other_property');?>:</td>
			<td>
				<?php if(get_cfg('info2.is_in_index')){?><input name="IsInIndex" type="checkbox" value="1" <?=$info_row['IsInIndex']=='1'?'checked':'';?>><?=get_lang('ly200.is_in_index');?><?php }?>
				<?php if(get_cfg('info2.is_hot')){?><input name="IsHot" type="checkbox" value="1" <?=$info_row['IsHot']=='1'?'checked':'';?>><?=get_lang('info2.is_hot');?><?php }?>
			</td>
		</tr>
	<?php }?>
	<tr>
		<td nowrap><?=get_lang('ly200.contents');?>:</td>
		<td class="ck_editor"><textarea class="ckeditor" name="Contents"><?=htmlspecialchars($db->get_value('info2_contents', "InfoId='$InfoId'", 'Contents'));?></textarea></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
